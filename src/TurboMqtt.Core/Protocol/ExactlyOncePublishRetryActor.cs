...
/// </summary>
internal sealed class ExactlyOncePublishRetryActor : UntypedActor
{
    /// <summary>
    ...
    public ExactlyOncePublishRetryActor(ChannelWriter<MqttPacket> outboundPackets, int maxRetries = PublishProtocolDefaults.DefaultMaxRetries,
        TimeSpan? publishTimeout = null)
    {
        _outboundPackets = outboundPackets;
        _maxRetries = maxRetries;
        _publishTimeout = publishTimeout ?? PublishProtocolDefaults.DefaultPublishTimeout;
    }
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            case PublishPacket packet:
            {
                ...
                _outboundPackets.TryWrite(packet); // we use unbounded channels - this won't fail
                _pendingPackets[packet.PacketId] = new PendingPublish(packet, Deadline.FromNow(TimeSpan.FromSeconds(5)),
                    Sender, false, 3);
                return;
            }
            ...
            case PublishProtocolDefaults.CheckPublishTimeout _:
            {
                foreach (var (packetId, pending) in _pendingPackets)
                {
                    ...
                    if (pending.RemainingRetries > 0)
                    {
                        ...
                        if (pending.PubRecReceived)
                        {
                            ...
                            _log.Debug("Publish packet with ID [{0}] timed out, resending PubRel", packetId);
                        }
                        else
                        {
                            ...
                            _log.Debug("Publish packet with ID [{0}] timed out, retrying", packetId);
                        }
                    }
                    else
                    {
                        // we've run out of retries
                        _log.Warning("Publish packet with ID [{0}] timed out, no more retries left", packetId);
